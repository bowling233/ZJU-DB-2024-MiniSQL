...
static LogRecPtr CreateInsertLog(txn_id_t txn_id, KeyType ins_key, ValType ins_val) {
  ...
  LogRecPtr log=std::make_shared<LogRec>(LogRecType::kInsert,LogRec::next_lsn_+1,txn_id,LogRec::GetPrevLSN(txn_id,LogRec::next_lsn_+1));
  LogRec::next_lsn_++;
  log->key[1]=ins_key;
  log->val[1]=ins_val;
  return log;
...
static LogRecPtr CreateDeleteLog(txn_id_t txn_id, KeyType del_key, ValType del_val) {
  LogRecPtr log=std::make_shared<LogRec>(LogRecType::kDelete,LogRec::next_lsn_+1,txn_id,LogRec::GetPrevLSN(txn_id,LogRec::next_lsn_+1));
  LogRec::next_lsn_++;
  log->key[2]=del_key;
  log->val[2]=del_val;
  return log;
...
static LogRecPtr CreateUpdateLog(txn_id_t txn_id, KeyType old_key, ValType old_val, KeyType new_key, ValType new_val) {
  LogRecPtr log=std::make_shared<LogRec>(LogRecType::kUpdate,LogRec::next_lsn_+1,txn_id,LogRec::GetPrevLSN(txn_id,LogRec::next_lsn_+1));
  LogRec::next_lsn_++;
  log->key[0]=old_key;
  log->val[0]=old_val;
  log->key[3]=new_key;
...
static LogRecPtr CreateBeginLog(txn_id_t txn_id) {
  LogRecPtr log=std::make_shared<LogRec>(LogRecType::kBegin,LogRec::next_lsn_+1,txn_id,LogRec::GetPrevLSN(txn_id,LogRec::next_lsn_+1));
  LogRec::next_lsn_++;
  return log;
}
...
static LogRecPtr CreateCommitLog(txn_id_t txn_id) {
  LogRecPtr log=std::make_shared<LogRec>(LogRecType::kCommit,LogRec::next_lsn_+1,txn_id,LogRec::GetPrevLSN(txn_id,LogRec::next_lsn_+1));
  LogRec::next_lsn_++;
  return log;
}
...
static LogRecPtr CreateAbortLog(txn_id_t txn_id) {
  LogRecPtr log=std::make_shared<LogRec>(LogRecType::kAbort,LogRec::next_lsn_+1,txn_id,LogRec::GetPrevLSN(txn_id,LogRec::next_lsn_+1));
  LogRec::next_lsn_++;
  return log;
}
